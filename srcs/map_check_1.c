...
/*   Updated: 2022/07/18 10:34:23 by sehattor         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void bfs_utils(t_so_long *sl, int **map, t_list **lst, int *item_g_sum)
{
	...
	while (++c < 4)
	{
		if (sl->gm.map[p.first + dx[c]][p.second + dy[c]] != WALL && sl->gm.map[p.first + dx[c]][p.second + dy[c]] != GOAL && map[p.first + dx[c]][p.second + dy[c]] == 0)
		{
			ft_lstadd_back(lst,
